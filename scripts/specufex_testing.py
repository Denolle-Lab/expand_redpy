...
#Kmeans
K_list=[5,6,7,8] # number of clusters to fit
...
def maxwindow(st,winlen): #provide stream object (one trace each) and window length in s
    ...
    tr_trim = final_tr.trim(stt+inx,stt+inx+winlen) #use inx to trim the window corretly
...
for vv in range(0,5):
    ...
    for i in range(1,len(tdf['waveform'])):
        ...
        if l != 1601: #if it is not 1601 (40fs over 40 s)
    ...
    #     print((y_pred))
        ...
        tdf['Kmeans'] = list(y_pred) #add Kmeans cluster to df
        test = tdf.drop(labels=["waveform","stft"],axis=1) #drop waveform and stft for saving
