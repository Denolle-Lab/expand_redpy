...
h5name = f'normalized_{volc.lower()}_templates_*.h5' #to be used in glob to get list of files at that volcano
...
waveforms = []
template_name = []
for filename in glob(f'{savedir}{h5name}'):
    with h5py.File(filename, "r") as f: #read file
        waves = f["waveforms"][()] #pull in waveforms
        temps = f["template_name"][()] #pull in waveform id
    [waveforms.append(i) for i in waves]
    [template_name.append(str(i)[2:-1]) for i in temps]

print(f"{len(waveforms)} waveforms in file")
# print(waveforms[0])
# print(template_name[0], type(template_name[0]))
...
save_path = f'{savedir}new_{h5name[:-5]}_{num_states}_states.h5'
